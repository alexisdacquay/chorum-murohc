...
| No password change or reset path | [#129](https://github.com/alexisdacquay/chorum-murohc/issues/129) | No longer true. Delivered: a parent changes their own password with their own current password (`POST /api/v1/auth/password/`); a parent resets a different household member's password with their own PIN or account password as proof (`POST /api/v1/household-members/<id>/reset-password/`). Both write an `account.*` or `password.*` audit event; resetting a parent's password also clears their PIN and lockout, per `_docs/approval-authentication.md` | Closed |
| No way to switch between households in one session | [#130](https://github.com/alexisdacquay/chorum-murohc/issues/130) | No longer true. Delivered: `GET`/`POST /api/v1/auth/household/` list and select among the caller's own live memberships only; a foreign or unknown id is refused identically (`NotFound`); zero or ambiguous unresolved context still fails closed exactly as before | Closed |
