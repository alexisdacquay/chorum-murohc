...
> `1ec2d24`. Nothing is fixed here; a gap is recorded, not closed. The two
> documentation gaps it recorded were then closed elsewhere, in issue #158;
> the rows below say where.
...
The go was on the product, not on the paperwork: `_docs/plan.md` stated three
sets of terms the product deliberately no longer followed, recorded below as
P-02. Issue #158 amended the plan, so it now states the approved interest,
reward and levelling rules and names the issue that approved each one.
...
| Interest on unspent points | Done | `_docs/interest-policy.md` replaced the plan's 20 percent monthly, daily, level-scaled draft with a flat weekly 2 percent capped at 20 points. Approved on #49; the plan text was updated to match in #158 |
| Rewards: 1 point per minute of game time, 200 points for GBP 5 | Done, terms replaced | #55 replaced the two fixed conversions with a parent-defined reward catalogue. The `reward` journey proves the spend and the exact ledger effect. The plan text was updated to match in #158 |
| Levelling consumes points, 500 for level 1, 30 to 40 levels | Done, terms replaced | #61 replaced spending with earning: ten levels from lifetime points, thresholds 50 to 2750, spending never demotes. The `creature` journey proves the level and its celebration. The plan text was updated to match in #158 |
...
| P-02 | Closed in [#158](https://github.com/alexisdacquay/chorum-murohc/issues/158). `_docs/plan.md` now states the weekly interest rule, the parent-defined reward catalogue and the earned ten-level scale, each naming the issue that approved it | [#158](https://github.com/alexisdacquay/chorum-murohc/issues/158) |
| P-03 | Closed in [#158](https://github.com/alexisdacquay/chorum-murohc/issues/158). `_docs/tasks.md` now opens with a note mapping all three never-written policy documents - reward, levelling and onboarding - to the issue or document that holds the approved decision. The briefs themselves are left as the historical starting points they are | [#158](https://github.com/alexisdacquay/chorum-murohc/issues/158) |
